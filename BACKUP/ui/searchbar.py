...
class SearchBar(QWidget):
    ...
    def show_bar(self, w):
        parent = self.parent()
        cfg = getattr(parent, "cfg", {}) if parent else {}
        theme_color = cfg.get("theme_color", "#4c1d95")
        theme_color2 = cfg.get("theme_color2", "#7c3aed")
        self._container.setStyleSheet(
            f"QWidget{{background:rgba(13,9,35,235);border:1.5px solid {theme_color2};border-radius:12px;}}")
        self._list.setStyleSheet(
            f"background:rgba(10,7,28,245);border-radius:10px;border:1px solid {theme_color};")
        ...
        try:
            ...
            for fp in self._res:
                ...
                btn.setStyleSheet(
                    ...
                    f"QPushButton:hover{{background:{theme_color2};}}")
                btn.setToolTip(fp)
