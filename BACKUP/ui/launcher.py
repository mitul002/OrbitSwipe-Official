...
class Launcher(QWidget):
    ...
    def _check_trial(self):
        ...
        # CASE 1: App is Licensed -> Periodic Silent Background Validation
        if st["licensed"]:
            now = time.time()
            # 6 hours silent background validation sync (21600 seconds)
            if now - getattr(self, "_last_license_check", 0) > 21600:
                self._last_license_check = now
                def _run_silent_check():
                    try:
                        from orbitswipe.core.license import check_license_online_silent
                        res = check_license_online_silent()
                        if res.get("status") == "revoked":
                            _log("Silent check: License was explicitly revoked online. Triggering UI lockout.")
                            QTimer.singleShot(0, self._lock_revoked_app)
                    except Exception as e:
                        _log(f"Silent background check exception: {e}")
                import threading
                threading.Thread(target=_run_silent_check, daemon=True).start()
            return
...
class OverlayPanel(QWidget):
    ...
    def _make_card(self, title_icon, title_text, width=360, height=330):
        ...
        theme_color, theme_color2 = self._get_colors()
        ss = f"""
            QFrame#ocard {{
                background: qlineargradient(x1:0,y1:0,x2:0,y2:1,
                    stop:0 rgba(16,12,45,252), stop:1 rgba(8,5,25,255));
                border: 1.5px solid {self._rgba(theme_color2, 200)};
                border-radius: 18px;
            }}
            QLineEdit, QTextEdit {{
                background: rgba(255,255,255,10);
                color: #e2e8f0;
                border: 1px solid {self._rgba(theme_color2, 80)};
                border-radius: 7px;
                font-family: 'Segoe UI';
                font-size: 10pt;
                padding: 4px 8px;
                selection-background-color: {theme_color2};
            }}
            QLineEdit:focus, QTextEdit:focus {{
                border: 1px solid {self._rgba(theme_color2, 220)};
                background: rgba(255,255,255,14);
            }}
        """
        ...
        sep.setStyleSheet(f"background:{self._rgba(theme_color2, 80)};max-height:1px;")
        cl.addWidget(sep)
    ...
    def _build_power(self, mode="shutdown"):
        ...
        # Switcher style
        PURPLE_TAB = (f"QPushButton{{background:{self._rgba(theme_color2, 50)};color:#c4b5fd;border-radius:7px;"
                      f"font-family:'Segoe UI';font-size:11pt;border:1px solid {self._rgba(theme_color2, 70)};}}"
                      f"QPushButton:checked{{background:{self._rgba(theme_color2, 180)};color:#fff;}}"
                      f"QPushButton:hover{{background:{self._rgba(theme_color2, 110)};}}")
        ...
        trow = QHBoxLayout()
        trow.setSpacing(6)
        TTAB = (f"QPushButton{{background:{self._rgba(theme_color2, 40)};color:#c4b5fd;border-radius:7px;"
                f"font-family:'Segoe UI';font-size:9pt;border:1px solid {self._rgba(theme_color2, 60)};}}"
                f"QPushButton:checked{{background:{self._rgba(theme_color2, 160)};color:#fff;}}"
                f"QPushButton:hover{{background:{self._rgba(theme_color2, 100)};}}")
        ...
        self._pw_reset_btn.setStyleSheet(
            f"QPushButton{{background:{self._rgba(theme_color2, 30)};color:#c4b5fd;"
            f"border:1px solid {self._rgba(theme_color2, 60)};border-radius:6px;"
            f"font-family:'Segoe UI';font-size:9pt;font-weight:bold;}}"
            f"QPushButton:hover{{background:{self._rgba(theme_color2, 80)};color:#fff;}}"
        )
        ...
        for lbl, m in [("5m", 5), ("15m", 15), ("30m", 30), ("1h", 60), ("2h", 120)]:
            ...
            pb.setStyleSheet(f"QPushButton{{background:{self._rgba(theme_color2, 40)};color:#c4b5fd;border-radius:5px;"
                             f"font-family:'Segoe UI';font-size:8pt;border:1px solid {self._rgba(theme_color2, 60)};}}"
                             f"QPushButton:hover{{background:{self._rgba(theme_color2, 120)};color:#fff;}}")
        ...
        self._pw_go_btn.setStyleSheet(
            f"QPushButton{{background:{self._rgba(theme_color2, 200)};color:#fff;border-radius:8px;"
            f"font-family:'Segoe UI';font-size:10pt;font-weight:bold;"
            f"border:1px solid {self._rgba(theme_color2, 255)};}}"
            f"QPushButton:hover{{background:{theme_color};}}")
        ...
        dynamic_btn_ss = (f"QPushButton{{background:{self._rgba(theme_color2, 80)};color:#e2e8f0;"
                          f"border-radius:8px;font-family:'Segoe UI';font-size:9pt;"
                          f"border:1px solid {self._rgba(theme_color2, 120)};padding:0 10px;}}"
                          f"QPushButton:hover{{background:{self._rgba(theme_color2, 180)};color:#fff;}}")
        
        self._pw_cancel_btn.setStyleSheet(dynamic_btn_ss)
    ...
    def _update_awake_btn_style(self):
        ...
        if self._awake_active:
            ...
            self._awake_btn.setStyleSheet(
                f"QPushButton{{background:{self._rgba(theme_color2, 80)};color:#e2e8f0;border-radius:8px;"
                f"font-family:'Segoe UI';font-size:10pt;font-weight:bold;"
                f"border:1px solid {self._rgba(theme_color2, 120)};}}"
                f"QPushButton:hover{{background:{self._rgba(theme_color2, 180)};color:#fff;}}")
