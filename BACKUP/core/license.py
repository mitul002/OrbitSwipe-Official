...
def _validate_license_online(key, email=None, request_transfer=False):
    try:
        import urllib.request
        payload_dict = {"key": key, "machine_id": _machine_id(), "app": "OrbitSwipe"}
        if email:
        ...
        with urllib.request.urlopen(req, timeout=8) as resp:
            ...
            if valid and not is_transfer:
                ...
                msg = f"True:{key}:{payload_dict['machine_id']}"
...
def _validate_license_offline(key):
    ...
    return {"valid": True, "message": "License valid (offline).", "plan": data.get("plan", "Pro")}
...
def activate_license(key, email=None, request_transfer=False):
    ...
    if result.get("valid") and not request_transfer:
        ...
        _log(f"License activated: {key[:8]}…")
    return result
...
def deactivate_license():
    # 1. Delete AppData file
    try:
        if os.path.exists(LICENSE_FILE): os.remove(LICENSE_FILE)
    except Exception as e: _log(f"deactivate file: {e}")
    
    # 2. Delete Registry mirror
    try:
        import winreg
        loc = _get_lic_stealth_loc()
        # Open and delete the subkey (since it's a simple key without subkeys)
...
def check_license_online_silent():
    ...
    # CASE C: User is Offline / Network Error
        ...
        _log("Silent online sync failed (Offline). Approving lifetime offline launch.")
